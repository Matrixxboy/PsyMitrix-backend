...
def generate_personality_pdf(filename, data, person_name="Parth"):
    ...
    toc_data = [
        ...
        ["9.", "Comparison of Traits (Matrix)", 11],
    ...
    story.append(Paragraph("6. Radar Chart of Traits", styles["SectionHeader"]))
    story.append(Paragraph(data["sections"]["charts"]["radarChart"]["explanation"], styles["Body"]))
    story.append(Spacer(1, 0.5*cm))
    story.append(RLImage(radar_path, width=12*cm, height=12*cm, hAlign='CENTER'))
    story.append(PageBreak())

    # ----- 7. Bar Chart Summary (Core Attributes) (Page 9) -----
    story.append(Paragraph("7. Bar Chart Summary (Core Attributes)", styles["SectionHeader"]))
    story.append(Paragraph(data["sections"]["charts"]["barChart"]["explanation"], styles["Body"]))
    story.append(Spacer(1, 0.5*cm))
    story.append(RLImage(bar_path, width=14*cm, height=7*cm, hAlign='CENTER'))
    story.append(PageBreak())

    # ----- 8. Cognitive Score Chart (New Chart Page 10) -----
    story.append(Paragraph("8. Cognitive Score Chart", styles["SectionHeader"]))
    story.append(Paragraph(data["sections"]["cognitive_scores"]["barChart"]["explanation"], styles["Body"]))
    story.append(Spacer(1, 0.5*cm))
    story.append(RLImage(cognitive_bar_path, width=14*cm, height=7*cm, hAlign='CENTER'))
    story.append(PageBreak())

    # ----- 9. Comparison of Traits (Table Page 11) -----
    story.append(Paragraph("9. Comparison of Traits (Matrix)", styles["SectionHeader"]))
    story.append(Paragraph("The following table benchmarks your core trait scores against the average population benchmark (Score of 5).", styles["Body"]))
    story.append(Spacer(1, 0.3*cm))
    comp = data["sections"]["charts"]["comparisonTable"]
    comp_rows = [["Trait Field", "Score (1-10)"]]
    for e in comp["data"]:
        # Ensure bolding works by embedding the tag in the string
        comp_rows.append([e["field"], f"{str(e['value'])}"]) 
    
    comp_tbl = Table(comp_rows, colWidths=[300, 120])
    comp_tbl.setStyle(TableStyle([
        ...
        ('GRID', (0, 0), (-1, -1), 0.5, colors.HexColor(SECONDARY_COLOR)),
        ('BACKGROUND', (0, 1), (-1, -1), colors.HexColor("#F7F9F7")), 
        ('ROWBACKGROUNDS', (0, 1), (-1, -1), [colors.white, colors.HexColor(ACCENT_COLOR)]), 
        ('FONTSIZE', (0,0), (-1,-1), 10),
        ('LEFTPADDING', (0, 0), (0, -1), 10)
    ]))
    story.append(comp_tbl)
    story.append(Spacer(1, 0.4*cm))
    story.append(Paragraph(comp.get("explanation", ""), styles["Body"]))
    story.append(PageBreak())
...
if __name__ == "__main__":
    ...
    generate_personality_pdf("Parth_Personality_Report_Fancier_V5.pdf", data, person_name="Parth")
